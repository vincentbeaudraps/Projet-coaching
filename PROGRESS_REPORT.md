...
**Date**: 7 février 2026  
**Phase**: 2 - Refactorisation Backend/Frontend
...
- ✅ `backend/src/routes/sessions.ts` - **2 routes migrées**
  - POST / (création séance avec validation)
  - Reste: 8 routes à migrer

**Économie estimée:** ~50 lignes (-8%)
...
### Backend Routes (70% restant)
**Priorité 1:**
- ⏳ `sessions.ts` - Compléter 8 routes restantes
- ⏳ `activities.ts` - ~5 routes (265 lignes totales)
- ⏳ `goals.ts` - ~4 routes

**Priorité 2:**
- ⏳ `training-plans.ts` - ~6 routes
- ⏳ `feedback.ts` - ~3 routes
- ⏳ `messages.ts` - ~4 routes
- ⏳ `performance.ts` - ~5 routes

**Économie potentielle:** ~600 lignes
