...
router.get('/me/records', authenticateToken, async (req, res) => {
  try {
    const userId = req.userId;
...
router.post('/me/records', authenticateToken, async (req, res) => {
  try {
    const userId = req.userId;
    const {
      distance_type,
    ...
    if (!distance_type || !distance_km || !time_seconds || !date_achieved) {
      return res.status(400).json({ 
        message: 'distance_type, distance_km, time_seconds, and date_achieved are required' 
      });
    }
    ...
    res.status(500).json({ message: 'Failed to add personal record' });
  }
});

// Update a personal record
router.put('/me/records/:recordId', authenticateToken, async (req, res) => {
  try {
    const userId = req.userId;
...
router.delete('/me/records/:recordId', authenticateToken, async (req, res) => {
  try {
    const userId = req.userId;
    const { recordId } = req.params;
...
router.get('/me/races', authenticateToken, async (req, res) => {
  try {
    const userId = req.userId;
...
router.post('/me/races', authenticateToken, async (req, res) => {
  try {
    const userId = req.userId;
    const {
      name,
    ...
    if (!name || !date || !distance_km) {
      return res.status(400).json({ 
        message: 'name, date, and distance_km are required' 
      });
    }
    ...
    res.status(500).json({ message: 'Failed to add upcoming race' });
  }
});

// Update an upcoming race
router.put('/me/races/:raceId', authenticateToken, async (req, res) => {
  try {
    const userId = req.userId;
    ...
    if (ownershipCheck.rows.length === 0) {
      return res.status(404).json({ message: 'Race not found or unauthorized' });
    }
    ...
    res.status(500).json({ message: 'Failed to update upcoming race' });
  }
});

// Delete an upcoming race
router.delete('/me/races/:raceId', authenticateToken, async (req, res) => {
  try {
    const userId = req.userId;
    const { raceId } = req.params;
    ...
    res.status(500).json({ message: 'Failed to delete upcoming race' });
  }
});
...
router.get('/me/annual-volumes', authenticateToken, async (req, res) => {
  try {
    ...
    console.error('Fetch annual volumes error:', error);
    res.status(500).json({ message: 'Failed to fetch annual volumes' });
  }
});

// Add or update annual volume
router.post('/me/annual-volumes', authenticateToken, async (req, res) => {
  try {
    const userId = req.userId;
    ...
    if (!year || volume_km === undefined) {
      return res.status(400).json({ 
        message: 'year and volume_km are required' 
      });
    }
    ...
    console.error('Add/update annual volume error:', error);
    res.status(500).json({ message: 'Failed to save annual volume' });
  }
});

// Delete annual volume
router.delete('/me/annual-volumes/:year', authenticateToken, async (req, res) => {
  try {
    const userId = req.userId;
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({ message: 'Annual volume not found' });
    }

    res.json({ message: 'Annual volume deleted successfully' });
  } catch (error) {
    console.error('Delete annual volume error:', error);
    res.status(500).json({ message: 'Failed to delete annual volume' });
  }
});

// Get athlete detailed statistics (for coach)
router.get('/:athleteId/detailed-stats', authenticateToken, authorizeRole('coach'), async (req, res) => {
  try {
    const { athleteId } = req.params;
    const coachId = req.userId;
    ...
    res.status(500).json({ message: 'Failed to fetch athlete statistics' });
  }
});

// Get athlete weekly progression (for coach)
router.get('/:athleteId/weekly-progression', authenticateToken, authorizeRole('coach'), async (req, res) => {
  try {
    const { athleteId } = req.params;
    const coachId = req.userId;
    ...
    console.error('Fetch weekly progression error:', error);
    res.status(500).json({ message: 'Failed to fetch weekly progression' });
  }
});
