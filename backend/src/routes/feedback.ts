...
router.post('/', authenticateToken, async (req: Request, res: Response) => {
  ...
  if (feelingRating && (feelingRating < 1 || feelingRating > 5)) {
    return res.status(400).json({ error: 'Feeling rating must be between 1 and 5' });
  }
  ...
  if (fatigueRating && (fatigueRating < 1 || fatigueRating > 5)) {
    return res.status(400).json({ error: 'Fatigue rating must be between 1 and 5' });
  }
  ...
  try {
    ...
    if (sessionResult.rows[0].athlete_id !== athleteId) {
      return res.status(403).json({ error: 'Not authorized to submit feedback for this session' });
    }
    ...
    if (coachResult.rows.length > 0) {
      ...
      const athleteName = coachResult.rows[0].athlete_name;
    ...
    res.status(500).json({ error: 'Failed to create/update feedback' });
  }
});
...
router.get('/athlete/:athleteId', authenticateToken, async (req: Request, res: Response) => {
  ...
  try {
    // Verify user is the athlete or their coach
    ...
    res.status(500).json({ error: 'Failed to fetch athlete feedback' });
  }
});
 ...
 */
router.patch('/:id/coach-comment', authenticateToken, async (req: Request, res: Response) => {
  const { id } = req.params;
  const { coachComment } = req.body;
  const coachId = req.userId;

  if (!coachComment) {
    return res.status(400).json({ error: 'Coach comment is required' });
  }
  ...
  try {
    ...
    const athleteId = feedbackResult.rows[0].athlete_id;
    const athleteName = feedbackResult.rows[0].athlete_name;
    const sessionTitle = feedbackResult.rows[0].title;
    ...
    res.status(500).json({ error: 'Failed to add coach comment' });
  }
});
...
router.delete('/:id', authenticateToken, async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (feedbackResult.rows[0].coach_comment) {
      return res.status(400).json({ error: 'Cannot delete feedback after coach has commented' });
    }
