...
// Delete session
router.delete('/:sessionId', authenticateToken, authorizeRole('coach'), asyncHandler(async (req: Request, res: Response) => {
  const { sessionId } = req.params;
...
router.get('/:sessionId/export/:format', authenticateToken, asyncHandler(async (req: Request, res: Response) => {
  ...
  switch (format.toLowerCase()) {
    ...
    default:
      throw new BadRequestError('Format not supported. Use: tcx, json, txt, or md');
  }
}));
